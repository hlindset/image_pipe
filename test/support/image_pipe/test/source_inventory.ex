defmodule ImagePipe.Test.SourceInventory do
  @moduledoc """
  Single source of truth for the committed image test sources
  ...
  **The verifiable facts are drift-checked** by
  ...
  libvips bump, a content change) is a deliberate act: update the entry and
  review the consumers below.

  `produced_by`:
    * `:gen_sources` — emitted by `mix fixtures.gen_sources` (the only way to
  ...
  @entries [
             ...
             %{
               ...
               invariant:
                 "Heavy-downscale resample-skew source; its diffuse skew calibrates the zone-plate tols. Also the quality-sensitive fixture for the SSIMULACRA2 metric + encode-search unit tests (incl. the native-JXL butteraugli max_bytes self-cap test)."
             },
             %{
               file: "high_freq.webp",
               width: 1600,
