for {grammar, crop_request, suffix} <- [
      {ImagePipe.Parser.Imgproxy.OptionGrammar, ImagePipe.Parser.Imgproxy.CropRequest, Framework},
      {ImagePipe.Dialect.Imgproxy.OptionGrammar, ImagePipe.Dialect.Imgproxy.CropRequest, Dialect}
    ] do
  defmodule Module.concat(ImgproxyOptionGrammarTest, suffix) do
    ...
    property "zoom aliases parse equivalent zoom_x and zoom_y assignments" do
      check all x_int <- integer(1..2000),
        ...
        assert @grammar.parse("zoom:#{x}:#{y}") == @grammar.parse("z:#{x}:#{y}")
      end
    end
    ...
    property "fixed-arity option aliases parse equivalently to their long forms" do
      ...
      check all {long, short} <- member_of(pairs),
                ...
                max_runs: 300 do
        assert @grammar.parse("#{long}:#{value}") == @grammar.parse("#{short}:#{value}")
      end
    end

    test "resize full grammar preserves explicit extend_requested assignment" do
      assert @grammar.parse("resize:fill:300:200:1:0") ==
               {:ok,
                {:pipeline,
                 [
                   resizing_type: :fill,
    ...
    test "gravity offsets parse pixels, zero pixels, and scale offsets" do
      assert @grammar.parse("g:soea:12:-0.25") ==
               {:ok,
                {:pipeline,
                 [
      ...
      assert @grammar.parse("gravity:ce:0:0") ==
               {:ok,
                {:pipeline,
                 [
    ...
    test "crop focal point and relative offsets parse into crop requests" do
      assert @grammar.parse("c:100:100:fp:0.25:0.75") ==
               {:ok,
                {:pipeline,
                 [
                   crop: %@crop_request{
      ...
      assert @grammar.parse("crop:100:200:nowe:0.25:-0.5") ==
               {:ok,
                {:pipeline,
                 [
                   crop: %@crop_request{
    ...
    test "object gravity parses tail tokens as class names" do
      assert @grammar.parse("g:obj:face") ==
               {:ok,
                {:pipeline,
                 [
                   gravity: {:obj, ["face"]},
      ...
      assert @grammar.parse("g:obj:face:5:5") ==
               {:ok,
                {:pipeline,
                 [
    ...
    test "crop object gravity parses tail tokens as class names" do
      assert @grammar.parse("c:100:100:obj:face") ==
               {:ok,
                {:pipeline,
                 [
                   crop: %@crop_request{
    ...
    test "crop object gravity with multiple classes parses all tail tokens as class names" do
      assert @grammar.parse("c:100:100:obj:cat:dog") ==
               {:ok,
                {:pipeline,
                 [
                   crop: %@crop_request{
    ...
    test "extend gravity keeps rejecting object gravity" do
      assert {:error, _} = @grammar.parse("extend:1:obj:face")
      assert {:error, _} = @grammar.parse("ex:1:obj")
    end
    ...
    test "extend gravity rejects focal-point gravity (diverges from imgproxy)" do
      ...
      assert {:error, _} = @grammar.parse("ex:1:fp:0.3:0.7")
      assert {:error, _} = @grammar.parse("extend:1:fp:0.5:0.5")
    end

    test "malformed padding and background options keep current error shapes" do
      assert @grammar.parse("padding:1:2:3:4:5") ==
               {:error, {:invalid_option_segment, "padding:1:2:3:4:5"}}

      assert @grammar.parse("padding:-1") ==
               {:error, {:invalid_option_segment, "padding:-1"}}

      assert @grammar.parse("padding:one") ==
               {:error, {:invalid_option_segment, "padding:one"}}

      assert @grammar.parse("background:256:0:0") ==
               {:error, {:invalid_background, ["256", "0", "0"]}}

      assert @grammar.parse("background:1:2") ==
               {:error, {:invalid_background, ["1", "2"]}}

      assert @grammar.parse("background:1::2") ==
               {:error, {:invalid_background, ["1", "", "2"]}}

      assert @grammar.parse("background:ffff") ==
               {:error, {:invalid_background, "ffff"}}

      assert @grammar.parse("background_alpha:") ==
               {:error, {:invalid_background_alpha, [""]}}

      assert @grammar.parse("bga:1.1") ==
               {:error, {:invalid_background_alpha, "1.1"}}
    end

    test "background alpha accepts fully transparent zero" do
      assert @grammar.parse("bga:0") ==
               {:ok, {:pipeline, [background_alpha: {:ratio, 0, 1}]}}

      assert @grammar.parse("bga:0.0") ==
               {:ok, {:pipeline, [background_alpha: {:ratio, 0, 10}]}}
    end

    test "basic effect options parse with imgproxy aliases" do
      assert @grammar.parse("blur:2.5") == {:ok, {:pipeline, [blur: 2.5]}}
      assert @grammar.parse("bl:3") == {:ok, {:pipeline, [blur: 3.0]}}
      assert @grammar.parse("bl:0") == {:ok, {:pipeline, [blur: 0.0]}}

      assert @grammar.parse("sharpen:0.7") == {:ok, {:pipeline, [sharpen: 0.7]}}
      assert @grammar.parse("sh:1") == {:ok, {:pipeline, [sharpen: 1.0]}}
      assert @grammar.parse("sh:0") == {:ok, {:pipeline, [sharpen: 0.0]}}

      assert @grammar.parse("pixelate:8") == {:ok, {:pipeline, [pixelate: 8]}}
      assert @grammar.parse("pix:12") == {:ok, {:pipeline, [pixelate: 12]}}
      assert @grammar.parse("pix:0") == {:ok, {:pipeline, [pixelate: 0]}}
    end

    test "brightness accepts imgproxy integer range -255..255" do
      assert @grammar.parse("br:255") == {:ok, {:pipeline, [brightness: 255]}}
      assert @grammar.parse("brightness:-255") == {:ok, {:pipeline, [brightness: -255]}}
      assert @grammar.parse("br:0") == {:ok, {:pipeline, [brightness: 0]}}
      assert @grammar.parse("br:256") == {:error, {:invalid_brightness, "256"}}
      assert @grammar.parse("br:1.5") == {:error, {:invalid_brightness, "1.5"}}
    end

    test "contrast accepts a positive float, 1 = unchanged" do
      assert @grammar.parse("co:1.5") == {:ok, {:pipeline, [contrast: 1.5]}}
      assert @grammar.parse("contrast:1") == {:ok, {:pipeline, [contrast: 1.0]}}
      assert @grammar.parse("co:0") == {:error, {:invalid_scale_factor, "0"}}
      assert @grammar.parse("co:-1") == {:error, {:invalid_scale_factor, "-1"}}
    end

    test "saturation accepts a positive float, 1 = unchanged" do
      assert @grammar.parse("sa:0.5") == {:ok, {:pipeline, [saturation: 0.5]}}
      assert @grammar.parse("saturation:2") == {:ok, {:pipeline, [saturation: 2.0]}}
      assert @grammar.parse("sa:0") == {:error, {:invalid_scale_factor, "0"}}
    end

    test "adjust fans out to brightness/contrast/saturation" do
      assert @grammar.parse("a:50") == {:ok, {:pipeline, [brightness: 50]}}
      assert @grammar.parse("a::1.5") == {:ok, {:pipeline, [contrast: 1.5]}}
      assert @grammar.parse("a:::0.8") == {:ok, {:pipeline, [saturation: 0.8]}}

      assert @grammar.parse("adjust:10:1.2:0.9") ==
               {:ok, {:pipeline, [brightness: 10, contrast: 1.2, saturation: 0.9]}}

      assert @grammar.parse("a:300") == {:error, {:invalid_brightness, "300"}}
      assert @grammar.parse("a::0") == {:error, {:invalid_scale_factor, "0"}}
    end

    test "adjust expands to the same effect assignments as the long forms" do
      assert @grammar.parse("a:50:1.5:0.8") ==
    ...
    test "dpr parses positive floats and rejects non-positive values" do
      ...
      assert @grammar.parse("dpr:1.5") == {:ok, {:pipeline, [dpr: 1.5]}}
      assert @grammar.parse("dpr:2") == {:ok, {:pipeline, [dpr: 2.0]}}
      assert @grammar.parse("dpr:0.5") == {:ok, {:pipeline, [dpr: 0.5]}}
      assert @grammar.parse("dpr:0") == {:error, {:invalid_positive_float, "0"}}
      assert @grammar.parse("dpr:-1") == {:error, {:invalid_positive_float, "-1"}}
      assert @grammar.parse("dpr:abc") == {:error, {:invalid_positive_float, "abc"}}
    end

    test "blur, sharpen, and pixelate reject invalid values with type-specific tags" do
      assert @grammar.parse("blur:-1") == {:error, {:invalid_non_negative_float, "-1"}}
      assert @grammar.parse("sharpen:abc") == {:error, {:invalid_non_negative_float, "abc"}}
      assert @grammar.parse("pixelate:-1") == {:error, {:invalid_non_negative_integer, "-1"}}
      assert @grammar.parse("pixelate:1.5") == {:error, {:invalid_non_negative_integer, "1.5"}}
    end

    test "tone effect options parse with imgproxy aliases" do
      assert @grammar.parse("monochrome:0.5") ==
               {:ok, {:pipeline, [monochrome: [intensity: {:ratio, 5, 10}]]}}

      assert @grammar.parse("mc:1:ffcc00") ==
               {:ok,
                {:pipeline,
                 [
                   monochrome: [
      ...
      assert @grammar.parse("mc:1:") ==
               {:ok, {:pipeline, [monochrome: [intensity: {:ratio, 1, 1}]]}}

      assert @grammar.parse("duotone:0.25") ==
               {:ok, {:pipeline, [duotone: [intensity: {:ratio, 25, 100}]]}}

      assert @grammar.parse("dt:0.5:112233") ==
               {:ok,
                {:pipeline,
                 [
                   duotone: [
      ...
      assert @grammar.parse("dt:0.5::ffeecc") ==
               {:ok,
                {:pipeline,
                 [
                   duotone: [
      ...
      assert @grammar.parse("dt:1:112233:ffeecc") ==
               {:ok,
                {:pipeline,
                 [
                   duotone: [
    ...
    test "invalid tone effect options return parser errors" do
      assert @grammar.parse("mc:") == {:error, {:invalid_option_segment, "mc:"}}
      assert @grammar.parse("mc:1.1") == {:error, {:invalid_intensity, "1.1"}}
      assert @grammar.parse("mc:0.5:zzzzzz") == {:error, {:invalid_monochrome, "zzzzzz"}}

      assert @grammar.parse("mc:0.5:ffffff:000000") ==
               {:error, {:invalid_option_segment, "mc:0.5:ffffff:000000"}}

      assert @grammar.parse("dt:0.5:fffxxx") == {:error, {:invalid_duotone, "fffxxx"}}

      assert @grammar.parse("dt:0.5:ffffff:zzzzzz") ==
               {:error, {:invalid_duotone, ["ffffff", "zzzzzz"]}}
    end

    test "colorize parses opacity, optional color, optional keep_alpha" do
      assert @grammar.parse("col:0.5") ==
               {:ok, {:pipeline, [colorize: [opacity: {:ratio, 5, 10}]]}}

      assert @grammar.parse("colorize:1:ff0000:1") ==
               {:ok,
                {:pipeline,
                 [colorize: [opacity: {:ratio, 1, 1}, color: color!(255, 0, 0), keep_alpha: true]]}}

      assert @grammar.parse("col:0.5:zzz") == {:error, {:invalid_colorize, "zzz"}}
    end

    test "gradient parses opacity + optional color/direction/start/stop" do
      assert @grammar.parse("gr:0.5") ==
               {:ok, {:pipeline, [gradient: [opacity: {:ratio, 5, 10}]]}}

      assert @grammar.parse("gradient:1:ff0000:left:0.25:0.75") ==
               {:ok,
                {:pipeline,
                 [
                   gradient: [
      ...
      assert @grammar.parse("gr:1::45") ==
               {:ok, {:pipeline, [gradient: [opacity: {:ratio, 1, 1}, angle: 45.0]]}}

      assert @grammar.parse("gr:0.5::sideways") == {:error, {:invalid_gradient, "sideways"}}

      # Invalid color → a bare-string payload, mirroring colorize (not list-wrapped).
      assert @grammar.parse("gr:0.5:zzz") == {:error, {:invalid_gradient, "zzz"}}

      # Out-of-range start/stop are rejected.
      assert @grammar.parse("gr:0.5::down:1.5") == {:error, {:invalid_gradient, "1.5"}}
    end
    ...
    test "invalid arity pipeline options return invalid option segment errors" do
      for segment <- invalid_pipeline_arity_segments() do
        assert @grammar.parse(segment) == {:error, {:invalid_option_segment, segment}}
      end
    end
    ...
    test "dropped options return unknown option errors" do
      for segment <- ~w(
        ...
        assert @grammar.parse(segment) == {:error, {:unknown_option, name}}
      end
    end
    ...
    test "objw gravity parses class/weight pairs (floats)" do
      assert @grammar.parse("g:objw:all:2:face:3") ==
               {:ok,
                {:pipeline,
                 [
    ...
    test "objw gravity accepts decimal weights" do
      assert {:ok, {:pipeline, opts}} = @grammar.parse("g:objw:face:2.5")
      assert opts[:gravity] == {:objw, [{"face", 2.5}]}
    end

    test "crop objw gravity parses class/weight pairs" do
      assert {:ok, {:pipeline, [crop: %@crop_request{gravity: gravity}]}} =
               @grammar.parse("c:100:100:objw:all:1:face:3")

      assert gravity == {:objw, [{"all", 1.0}, {"face", 3.0}]}
    end

    test "objw gravity rejects non-positive, odd-arity, empty-class, and bare forms" do
      assert {:error, _} = @grammar.parse("g:objw:face:0")
      assert {:error, _} = @grammar.parse("g:objw:face:-2")
      assert {:error, _} = @grammar.parse("g:objw:all:2:face")
      assert {:error, _} = @grammar.parse("g:objw::3")
      assert {:error, _} = @grammar.parse("g:objw")
    end
    ...
    test "objw gravity rejects weight above overflow-safe ceiling (1e308)" do
      assert {:error, _} = @grammar.parse("g:objw:cat:1e308")
      assert {:error, _} = @grammar.parse("g:objw:cat:1.0e308")
      assert {:error, _} = @grammar.parse("c:100:100:objw:cat:1e308")
    end

    test "objw gravity accepts large-but-safe weights below the ceiling" do
      assert {:ok, _} = @grammar.parse("g:objw:cat:1000000")
      assert {:ok, _} = @grammar.parse("g:objw:cat:999999.9")
    end
    ...
    test "parse_float-backed options do not raise on 320-digit tokens" do
      ...
      assert {:error, _} = @grammar.parse("g:objw:cat:#{huge}")

      # focal-point path (parse_focal_coordinate → parse_float)
      assert {:error, _} = @grammar.parse("g:fp:#{huge}:0.5")

      # zoom path (parse_positive_float → parse_float)
      assert {:error, _} = @grammar.parse("z:#{huge}")

      # dpr path (parse_positive_float → parse_float via @special_specs)
      assert {:error, _} = @grammar.parse("dpr:#{huge}")
    end

    describe "trim" do
      test "parses threshold only (smart background)" do
        assert {:ok, {:pipeline, [trim: trim]}} = @grammar.parse("trim:15")
      ...
      test "alias t with color and equal flags" do
        assert {:ok, {:pipeline, [trim: trim]}} = @grammar.parse("t:10:ff00ff:1:1")
      ...
      test "empty threshold disables trim (no assignment)" do
        assert {:ok, {:pipeline, []}} = @grammar.parse("trim:")
      end

      test "rejects more than 4 args" do
        assert {:error, _} = @grammar.parse("trim:10:ff00ff:1:1:0")
      end
      ...
      test "rejects more than 4 args even when threshold is empty" do
        ...
        assert {:error, _} = @grammar.parse("trim:::::")
      end

      test "rejects a bad threshold" do
        assert {:error, _} = @grammar.parse("trim:nope")
      end

      test "rejects a bad boolean (stricter than imgproxy, like enlarge)" do
        assert {:error, _} = @grammar.parse("trim:10::x")
      end
    end

    describe "max_bytes" do
      test "parses max_bytes and its mb alias into the output scope" do
        assert {:ok, {:output, [max_bytes: 51_200]}} = @grammar.parse("max_bytes:51200")
        assert {:ok, {:output, [max_bytes: 51_200]}} = @grammar.parse("mb:51200")
      end

      test "mb:0 disables the ceiling (no-op), matching imgproxy" do
        assert {:ok, {:output, [max_bytes: nil]}} = @grammar.parse("mb:0")
      end

      test "rejects negative or malformed max_bytes" do
        assert {:error, _} = @grammar.parse("mb:-5")
        assert {:error, _} = @grammar.parse("mb:abc")
      end
    end
    ...
    describe "autoquality" do
      test "autoquality:none disables the search" do
        assert {:ok, {:output, [quality_search: {:autoquality, :disabled}]}} =
                 @grammar.parse("autoquality:none")

        assert {:ok, {:output, [quality_search: {:autoquality, :disabled}]}} =
                 @grammar.parse("aq:none")
      end

      test "autoquality:size with full args" do
        assert {:ok, {:output, [quality_search: {:autoquality, fields}]}} =
                 @grammar.parse("autoquality:size:10240:10:80")
      ...
      test "autoquality:ssim2 with full args" do
        assert {:ok, {:output, [quality_search: {:autoquality, fields}]}} =
                 @grammar.parse("autoquality:ssim2:90:70:80:1")
      ...
      test "aq:butteraugli parses to a butteraugli quality search" do
        assert {:ok, {:output, [quality_search: {:autoquality, fields}]}} =
                 @grammar.parse("aq:butteraugli:1.0:75:95:0.1")
      ...
      test "bare dssim is accepted as an ssimulacra2 alias with no inline args" do
        assert {:ok, {:output, [quality_search: {:autoquality, fields}]}} =
                 @grammar.parse("autoquality:dssim")
      ...
      test "dssim with inline args is rejected" do
        assert {:error, _} = @grammar.parse("autoquality:dssim:0.02:70:80:0.001")
        assert {:error, _} = @grammar.parse("autoquality:dssim:90")
      end

      test "ml and unknown methods are rejected" do
        assert {:error, _} = @grammar.parse("autoquality:ml:0.02:70:80:0.001")
        assert {:error, _} = @grammar.parse("autoquality:bogus")
      end

      test "min greater than max is rejected" do
        assert {:error, _} = @grammar.parse("autoquality:size:10240:80:10")
      end
    end

    describe "debug" do
      test "debug:1 / debug:true parse a truthy response-scoped flag" do
        assert @grammar.parse("debug:1") == {:ok, {:response, [debug?: true]}}
        assert @grammar.parse("debug:true") == {:ok, {:response, [debug?: true]}}
      end

      test "debug:0 / debug:false parse an explicitly disabled flag" do
        assert @grammar.parse("debug:0") == {:ok, {:response, [debug?: false]}}
        assert @grammar.parse("debug:false") == {:ok, {:response, [debug?: false]}}
      end

      test "debug with a missing or malformed value is rejected" do
        assert {:error, {:invalid_option_segment, "debug:"}} = @grammar.parse("debug:")
        assert {:error, {:invalid_option_segment, "debug:x"}} = @grammar.parse("debug:x")
        assert {:error, {:invalid_option_segment, "debug:1:2"}} = @grammar.parse("debug:1:2")
      end
    end
    ...
    describe "codec encoder options" do
      ...
      test "jpgo omitted leading positions stay nil (omit-vs-false)" do
        ...
        assert {:ok,
                {:output, [encoder_options: %{jpeg: %JpegOptions{optimize_scans: true} = j}]}} =
                 @grammar.parse("jpgo:::::true")
      ...
      test "webpo compression emits both lossless bools explicitly (resettable)" do
        assert {:ok,
                {:output,
                 ...
                 @grammar.parse("webpo:near_lossless::photo")
      end

      test "avifo subsample -> subsample_mode" do
        assert {:ok, {:output, [encoder_options: %{avif: %AvifOptions{subsample_mode: :off}}]}} =
                 @grammar.parse("avifo:off")
      end

      test "full names parse identically to aliases" do
        assert @grammar.parse("jpeg_options:1") == @grammar.parse("jpgo:1")
        assert @grammar.parse("png_options::1:16") == @grammar.parse("pngo::1:16")
      end

      test "too many positionals is an invalid segment" do
        assert {:error, {:invalid_option_segment, _}} = @grammar.parse("avifo:off:extra")
      end
    end
