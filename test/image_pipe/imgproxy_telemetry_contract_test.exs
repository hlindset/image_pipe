# The telemetry contract for imgproxy requests, run against the inverted dialect
# stack (`ImagePipe.Dialect.Imgproxy`). `call_conn/2` drives the dialect plug.
#
...
defmodule ImagePipe.ImgproxyTelemetryContractTest do
  ...
  # A source with a STRONG byte identity, so the dialect emits an ETag and the
  # 304 scenario is constructible. `RootHTTPAdapter`'s `byte_identity: :none`
  # would withhold the ETag entirely — a wire-surface concern, not this file's
  # subject.
  ...
  # span wraps the adapter call, not the lazy enumeration) — a blocking point in
  # the fetch itself, distinct from the `:chain` seam.
  ...
  # Every stage the dialect can emit. Attaching on the full set (not on a
  # scenario's own subset) is what lets a single assertion say "this stage did
  # NOT fire" — a narrower list would make every refutation vacuous.
  @stages [
    [:request],
    [:parse],
    [:send],
  ...
  # Structural invariants that must hold of ANY request.
  ...
  # dialect `Pipeline.run/4`'s — the dialect's `init/1` accepts neither as a
  # known option, so they are appended after validation, the convention the
  # wire suite already uses.
  @test_only_seam_keys [:image_module, :chain]

  # The stack-invocation site: `call_conn/2` drives the dialect plug.
  ...
  describe "image cache miss" do
    test "emits the shared stage set, request outermost, in stage order", %{prefix: prefix} do
      ...
      # transform operation closes and BEFORE delivery opens — the post-clamp,
      # pre-encode position. A plain fit-resize has no materializing op, so the
      # span can only be the backstop.
  ...
  describe "304 not-modified" do
    test "resolves before the cache lookup and before the fetch", %{prefix: prefix} do
      ...
      # guidelines]. This is the assertion that would fail if the dialect
  ...
  describe "streamed error after preparation" do
    test "the delivery stage carries the failure as :result", %{prefix: prefix} do
      ...
      # A committed 200 whose stream then fails reports the mid-stream send
      # override as the request span's result.
  ...
  #
  # The `:chain` seam is `Dialect.Imgproxy.Pipeline.run/4`'s own injectable.
  # This is the scenario that would fail if the dialect's `[:source,
  # :fetch_decode]` span enclosed the build continuation: the forced transform
  # failure would land on fetch_decode's stop instead of the execute span's.
  ...
  # terminal. One assertion per remaining terminal.
...
defmodule ImagePipe.ImgproxyTelemetryStageSetTest do
  @moduledoc """
  Measures, rather than assumes, the stage sequence `Dialect.Imgproxy` emits for
  one plain image cache miss: it must be exactly the sequence the shipped
  `Dialect.Native` emits.

  Sequence, not set — the dialects must agree on stage ORDER too, and a set
  comparison would let a reordering through. This is the spec's actual telemetry
  claim, and it holds.
  """
