# Differential conformance against imgproxy's own baked fixtures, run against BOTH
# stacks: the framework arm (`ImagePipe.Plug` + `ImagePipe.Parser.Imgproxy`) and the
# inverted dialect arm (`ImagePipe.Dialect.Imgproxy`, which owns its whole request
# chain). Every constellation body below is written once and compiled twice; the only
# thing that differs is the arm `Harness.plug_opts/1` builds in `setup_all`.
#
# This is the project's strongest parity net: it compares real decoded pixels against
# fixtures baked by imgproxy itself. A parity bug in either stack surfaces here as
# exactly one red arm.
#
# TRAP, and the reason the arm is threaded as data rather than aliased: an
# `alias`/`import` naming one stack's plug would pin BOTH arms to that module. The
# suite would compile, the case count would double, and the framework arm would run
# twice under both names — green while proving nothing. The aliases below are shared
# support (constellations, manifest, pixel compare) and the arm-parameterized
# `Harness`; none of them names a stack.
#
# Never re-bake to make an arm pass: a dialect-arm failure is a dialect bug.
for {arm, suffix} <- [{:framework, Framework}, {:dialect, Dialect}] do
  defmodule Module.concat(ImagePipe.ImgproxyDifferentialConformanceTest, suffix) do
    ...
    setup_all do
      ...
      # drift rather than a regression; we record both and always compare. Printed once,
      # from the framework arm — the provenance is a property of the fixtures, not an arm.
      if @arm == :framework do
        IO.puts(
          :stderr,
      ...
      # One arm, built once per module and threaded through every render below. The two
      # arms are initialized independently and share no state (the differential harness
      # wires no cache at all), so neither arm can serve the other's bytes.
      {:ok, manifest: manifest, plug_opts: Harness.plug_opts(@arm)}
    end
    ...
    for constellation <- Constellations.all() do
      ...
      test "#{@arm}: #{@c.id} (#{@c.verdict}/#{@c.group})", ctx do
