defmodule ImagePipe.API.OrientationMatrixTest do
  @moduledoc """
  API orientation regression coverage for display-frame geometry and
  storage-frame shrink-on-load planning.
  """

  # Real fetch/decode through a Plug-backed origin per case — keep it serial,
  # mirroring pipeline_pixel_test.exs and api_wire_test.exs.
  ...
  # ── the matrix: {label, api-dialect option segments} ───────────────────
  #
  ...
  # ── wire-level helpers (mirrors api_wire_test.exs) ──────────────────
  ...
  describe "pixel invariance (wire-level twin oracle)" do
    for orientation <- @orientations, {label, segments} <- @matrix do
      test "EXIF #{orientation}: #{label} produces pixel-close wire output for twin vs oriented source" do
        ...
        label = unquote(label)
        path = api_path(segments)
        base_png = fixture_base_png()

        oriented_conn = get(path, api_opts({OrientedFrameOrigin, {base_png, orientation}}))
        twin_conn = get(path, api_opts({Orientation1TwinOrigin, {base_png, orientation}}))
