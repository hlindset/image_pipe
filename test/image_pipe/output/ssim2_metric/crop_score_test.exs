defmodule ImagePipe.Output.Ssim2Metric.CropScoreTest do
  ...
  describe "tile_coords/3" do
    ...
    test "both axes exactly at tile size yields a single tile covering the whole frame" do
      coords = CropScore.tile_coords(512, 512, 512)
      assert coords == [{0, 0, 512, 512}]
    end

    test "non-square with one axis an exact multiple and one not" do
      # 1536x600: x positions 0,512,1024 (exact multiple); y: 0,88 (600-512)
      coords = CropScore.tile_coords(1536, 600, 512)
      xs = coords |> Enum.map(fn {x, _y, _w, _h} -> x end) |> Enum.uniq() |> Enum.sort()
      ys = coords |> Enum.map(fn {_x, y, _w, _h} -> y end) |> Enum.uniq() |> Enum.sort()

      assert xs == [0, 512, 1024]
      assert ys == [0, 88]
      assert length(coords) == 6
    end

    test "frame smaller than tile size on both axes yields one clamped tile" do
      coords = CropScore.tile_coords(300, 200, 512)
      # clamped tile covers the whole (smaller) frame; width/height match frame dims
      assert coords == [{0, 0, 300, 200}]
    end
  end
  ...
  describe "subsample/2" do
    ...
    test "returns all items unchanged when count equals k exactly" do
      tiles = Enum.to_list(1..16)
      assert CropScore.subsample(tiles, 16) == tiles
    end

    test "returns a single-element list unchanged" do
      assert CropScore.subsample([:only], 16) == [:only]
    end
  end
  ...
  describe "percentile/2" do
    ...
    test "p=0.0 returns the first (lowest) element" do
      sorted = [1.0, 5.0, 10.0, 20.0]
      assert CropScore.percentile(sorted, 0.0) == 1.0
    end

    test "p=1.0 returns the last (highest) element" do
      sorted = [1.0, 5.0, 10.0, 20.0]
      assert CropScore.percentile(sorted, 1.0) == 20.0
    end

    test "p=0.5 returns the median-low element (floor-indexed)" do
      sorted = [10.0, 20.0, 30.0, 40.0]
      # floor(0.5 * 3) = floor(1.5) = 1 -> sorted[1] = 20.0
      assert CropScore.percentile(sorted, 0.5) == 20.0
    end
  end
  ...
  describe "tile_count/2" do
    ...
    test "a single-tile frame (both axes < tile size) scores exactly 1 tile" do
      assert CropScore.tile_count(300, 200) == 1
    end

    test "a frame exactly at tile size on both axes scores exactly 1 tile" do
      assert CropScore.tile_count(512, 512) == 1
    end
  end
