defmodule ImagePipe.Native.OffsetCanvasWireTest do
  ...
  @placement_source Path.expand(
                      "../../../support/image_pipe/test/imgproxy_differential/sources/placement.png",
                      __DIR__
                    )
  ...
  test "invalid offset and canvas combinations fail before source access" do
    config = native_config({CountingOriginImage, test_pid: self()})
  ...
  defp response(options, origin) do
    ...
    |> ImagePipe.Plug.call(native_config(origin))
  end

  defp native_config(origin) do
    ImagePipe.Plug.init(
      sources: [
        path:
          {RootHTTPAdapter,
