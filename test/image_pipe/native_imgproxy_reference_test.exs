defmodule ImagePipe.NativeImgproxyReferenceTest do
  @moduledoc """
  Compact pixel references baked by upstream imgproxy and exercised through
  ImagePipe's native request API.

  See `test/support/image_pipe/test/imgproxy_reference/README.md` for immutable
  fixture provenance and the native translation of each upstream request.
  """
  ...
  for {name, source, options} <- [
    ...
    test "native #{name} stays within the retained imgproxy pixel reference" do
