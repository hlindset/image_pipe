defmodule ImagePipe.APIErrorPathsTest do
  @moduledoc """
  ...
  Rows already covered by `ImagePipe.APIWireTest`'s "delivery
  ...
  #
  # Single-purpose fixtures live beside the tests that exercise them.
  ...
  # the runner's `config` since `build_and_pump/6` hands `config`
  ...
  #
  # `APIWireTest`'s "owner-kill" test (Task 15) kills the owner AFTER
  ...
  # the API — a raising first pull — mirroring
  ...
  #
  # `APIWireTest`'s "bracket cleanup runs exactly once after an explicit
  ...
  describe "row 10: pre-first-chunk post-transform exception (forced clamp)" do
    test "renders 500-class, opens no sink, cleans up once, and reports processing_error" do
      test_pid = self()
      prefix = [:"api_error_paths_clamp_#{System.unique_integer([:positive])}"]
      handler_id = "api-error-paths-clamp-#{inspect(prefix)}"
