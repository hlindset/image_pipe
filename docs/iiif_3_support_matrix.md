...
| `pct:n` | `sizeByPct` | ✅ | `Resize{mode: :fit, zoom_x: {:ratio, n, 100}, zoom_y: {:ratio, n, 100}}` (canonicalized; decimal `n` becomes an exact integer ratio, e.g. `pct:20.5` → `{:ratio, 205, 1000}`). The zoom is carried as an **exact ratio** rather than an `n/100` float, so the derived axis multiplies as `dim × num / den` and rounds half-away exactly at a tie — `pct:20.5` of a 200×300 source is `41×62` (`300 × 205/1000 = 61.5 → 62`), where a lossy float gave `61` ([#317](https://github.com/hlindset/image_pipe/issues/317)). The IIIF "`n` ≤ 100" rule is lifted under `^` (`^pct:200` is a valid upscale; bare `pct:200` is a 400). |
