...
- Modifies: `check_detector/2` mirrors `ImagePipe.Plug.validate_detector_capability/2` (plug.ex:184-196): under `detector_required: true` with detection requested, reject **only when** `Transform.detector_available?(config[:detector], opts_with_classes)` is false — availability is class-dependent (the gate-triad wire case has a Composite whose face child is available and object child is not: `g:obj:face` → 200, `g:obj:car` → 422). **The gate consults `detect_classes/1` only — deliberately NOT `face_assist?/1`**: the framework's gate checks `Plan.detect_classes(plan) != nil` alone (plug.ex:185-187), while face-assist participates only in cache-key identity (runner.ex:197). Adding face-assist to the gate would create a framework divergence, not close one. Pin the asymmetry: add a dual-run no-divergence wire case — face-assist smart crop (`g:sm` with `smart_crop_face_detection: true`) under `detector_required: true` + an unavailable face detector returns **200** (degrade) on both arms, not 422.
...
- Produces: `Identity.material(request, negotiation, conn, config, detector_identity)` — 5-arity, `detector_identity :: term() | nil`. The term goes into the **`representation`** list (`[detector: detector_identity]` appended alongside `output_policy`), NOT `storage_only` — that placement feeds both the ETag and the cache key (representation.ex:92-100), matching the framework where detector identity is ETag material (`etag_material/4` drops only `:cache`, http_cache.ex:59-77). **Append the entry only when `detector_identity` is non-nil**: `Representation.build` hashes the material as-is (representation.ex:87-100, no nil-dropping), so an always-present `detector: nil` entry would churn every dialect cache key AND ETag — a global reshape spec P7 scopes to *detection requests only*. Cross-stack key equality is a non-goal, so mirroring `cache/key.ex:58`'s unconditional entry buys nothing.
...
- [ ] **Step 4: Verify GREEN + add the ETag half.** Every un-gated assertion is cache-key-only (`lookup_key` via `CacheProbe`); spec exit criterion 2 names the **ETag** too, and key-equality cannot detect a `storage_only` misplacement (storage_only busts the key but not the ETag). Add: (i) unit — `identity_test.exs`: `material/5` with differing `detector_identity` values yields differing `representation` (hence differing built ETag); nil vs present differ; and **`material/5` with nil yields a representation with NO `:detector` key** — pinning that non-detection keys and ETags are byte-identical to the pre-B1b material (no global churn); (ii) wire — extend the identity cases to also capture `get_resp_header(conn, "etag")` and assert it varies with the relevant model and is invariant to the irrelevant one. Run the dialect identity unit tests (no expected-material updates should be needed for non-detection cases — if one changes, the no-churn pin is violated; stop and investigate).
...
- [ ] **Step 5: Sync docs + commit** (per the global per-commit doc rule — B3's doc duty is incremental across Tasks 7–10, not deferred to Task 11): update the matrix § Observability clamp line and `docs/telemetry.md`'s clamp emission-site prose in this commit.

```bash
git add lib/image_pipe/output/clamp.ex lib/image_pipe/request/delivery_build.ex test/ docs/
...
**Interfaces:**
- Produces: one shared negotiate function enclosing **both legs** — `Policy.resolve/2` AND the `:needs_final_image_alpha` second resolution (delivery_build.ex:335-346) — inside a single `[:output, :negotiate]` span with start `%{output_mode:}` and stop metadata built from the **final** resolved output (`%{result:, output_format:}`, delivery_build.ex:348-357). Signature sketch: `negotiate_output(policy, source_format, alpha_fun, telemetry_opts) :: {:ok, resolved} | {:error, reason}` where `alpha_fun :: (-> boolean)` defers the `Image.has_alpha?` probe to the caller (framework and dialects have different image handles at that point). Error-shape ownership (the three implementations differ): the helper returns `{:error, reason}` **unwrapped**; the framework's call site keeps its `{:error, {:output, reason}}` wrap (delivery_build.ex:344) and the dialects keep their unwrapped pass-through (imgproxy.ex:788-790) — each stack's observable error shape is unchanged. Error-path span semantics to pin explicitly: exactly ONE negotiate span per request regardless of outcome; the second resolution leg runs only on `{:needs_final_image_alpha, :source}`; a first-leg failure stops the span with its error `:result` (mirror the framework's current stop metadata for that path); a raise in `alpha_fun` follows `:telemetry.span`'s `:exception` semantics. Read all three current `resolve_output` implementations before fixing the signature — they must all collapse onto it without behavior change.
- Produces: `InputColorManagement.condition/2` emits the `[:transform, :input_color_management]` span itself via `state.telemetry_opts`, with stop `%{result:, working_space:, imported?:}`; `Executor.seed_color_management/2` stops wrapping. Exact semantics (executor.ex:90-113): **the span fires on every `condition/2` invocation — once per executed request on all stacks — INCLUDING when conditioning is a no-op** (stop metadata `imported?: false`), and the `working_space` value is computed in `run_color_management` *before* the condition call today, so that computation moves into `condition/2` with the span. The framework's `seed_orientation: false` planning path never calls `condition/2`, so span-on-planning cannot regress. Pin the no-op case: a non-profiled source emits the span with `imported?: false` on both arms.
...
- [ ] **Step 4: Sync docs + commit** (incremental B3 doc duty): matrix § Observability negotiate/ICM lines + `docs/telemetry.md` emission-site prose for both relocated spans.

```bash
git add lib/image_pipe/output/ lib/image_pipe/request/delivery_build.ex lib/image_pipe/dialect/ lib/image_pipe/transform/ test/ docs/
...
- [ ] **Step 4: Sync docs + commit** (incremental B3 doc duty: matrix Observability materialize line, `docs/telemetry.md` if its prose names the emitting stacks): `git add lib/image_pipe/dialect/ test/ docs/ && git commit -m "B3: materialize delivery backstop (and span) on both dialect build paths"`.
...
**Files:**
- Modify: `lib/image_pipe/dialect/imgproxy.ex` (send sites :217/:224/:246/:253/:549/:562/:698-704 + Task 4's OPTIONS/405 heads; `build_fun`/`build_and_pump` :724-764)
- Modify: `lib/image_pipe/dialect/native.ex` (mirror sites)
- Modify: `lib/image_pipe/decode.ex` (the fetch_decode span seam in `with_image`), possibly `lib/image_pipe/telemetry.ex` (thin start/stop helper beside `span/4`)
- Modify: `test/image_pipe/imgproxy_telemetry_contract_test.exs` (final `@framework_only` → `[]`; `@shared_stages` gains the four — both pin modules live in THIS one file: the dual-run contract module at the top, `ImagePipe.ImgproxyTelemetryStageSetTest` at :523)
...
- `[:source, :fetch_decode]`: **the span must close after decode, before the build continuation runs** — naively wrapping the `Decode.with_image` call (imgproxy.ex:729-740) is WRONG: `with_image` runs the transform/encode/pump callback inside the same bracket (decode.ex:82-88), so a whole-call span would misattribute transform/encode time and failures to fetch_decode and nest `[:transform, :execute]`/`[:encode]` under it, unlike the framework (whose span encloses only `Source.fetch` + decode/validation, processor.ex:60-68). The build callback must also STAY inside the `Source.with_fetched` bracket (the source handle must outlive the transform), so the span cannot be a plain `Telemetry.span/4` fn wrapper around fetch+decode. Design: emit the span from `Decode.with_image` itself (both dialects gain it from one seam; the framework keeps its own processor-side span) using manual start/stop with `:telemetry.span`'s event shapes and timing semantics — start before `Source.with_fetched`, stop inside its callback immediately after the decoded `State`/geometry are built (before `fun.(state, geometry)`), with an error stop if fetch or decode fails before that point and `:exception` semantics on raise. Stop metadata mirrors `fetch_decode_stop_metadata/1`. If a thin `Telemetry` start/stop helper is needed, add it beside `span/4`; if the enclosure proves intractable inside the bracket, **escalate** rather than shipping a span that excludes the fetch or encloses the build. Required assertions (contract test): `[:source, :fetch_decode] :stop` precedes `[:transform, :execute] :start`; `[:source, :fetch]` nests inside `[:source, :fetch_decode]` on both arms; and a transform-failure scenario leaves the fetch_decode span's `:result` non-error.
...
- [ ] **Step 3: Add focused `[:send]` coverage** — the plain-GET stage-set scenario cannot prove all terminal sites are wrapped. In the contract test, assert the send span (with `result`/`status` stop metadata) on: a successful image response, `/info`, a dialect error response, OPTIONS 204, and method 405; assert `[:deliver]` nests inside `[:send]` only on paths that stream through `Response.Sender`.

- [ ] **Step 4: Verify GREEN**: contract test `@framework_only == []`; stage-set sequence equality; error-path suites; `test/image_pipe/telemetry/trace/encode_span_test.exs`; the parentage suites again (`test/image_pipe/delivery/trace_parentage_test.exs`, `test/image_pipe/telemetry/delivery_span_parentage_baseline_test.exs`, `telemetry/trace/capture_test.exs`, `telemetry/logger_test.exs`) — this task changes dialect span nesting substantially and a wrong fetch_decode boundary could still satisfy a stage-set check; full wire + differential.
- [ ] **Step 5: Commit** (`git add lib/image_pipe/decode.ex lib/image_pipe/telemetry.ex lib/image_pipe/dialect/ test/ && git commit -m "B3: dialect-emitted send/fetch_decode/execute/encode spans; encode forces first chunk"`).
...
- [ ] **Step 3: Docs coherence sweep**: Tasks 7–10 updated `docs/telemetry.md` and the matrix incrementally; this step verifies the whole B3 doc surface reads coherently, closes the matrix § Observability stage-set row, and records B3 in the backlog.
