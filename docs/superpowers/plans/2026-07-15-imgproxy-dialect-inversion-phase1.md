...
- **Delete, applying that SAME rule (user ruling 2026-07-15 — this plan's original file list named only the supervisor test and missed these three, 33 tests whose subject this task deletes):** `test/image_pipe/request/source_session_test.exs` (25 tests), `test/image_pipe/request/vix_stream_continuation_test.exs` (4 — real-Vix continuation proofs), `test/image_pipe/request/source_session/producer_test.exs` (4), plus their support module. **The rule, stated once and applied to all four files:** port every case that covers behavior which *still exists* after the migration and is not asserted elsewhere (staging, owner-death-before-commit, error taxonomy, Vix stream continuation — all now `Delivery.Coordinator`'s behavior); delete only cases whose *subject* is genuinely gone (the supervisor itself: `assert_child_counts`, restart policy). A case is not redundant merely because the wire/differential suites also exercise the path — those are black-box; these pin process-level behavior on the repo's most safety-critical code. Same judgment for `request_runner_test.exs`'s ~25 `assert_supervisor_*` assertions: those whose subject is the supervisor die; those asserting delivery behavior get repointed.
...
- [ ] **Step 2: Migrate `runner.ex`.** Replace `SourceSessionSupervisor.start_session` + `SourceSession.prepare` with `ImagePipe.Delivery.stream(self(), build_fun, representation.cache_key, response_meta, opts)` where `build_fun` wraps the existing `Processor` fetch/decode/encode flow, modeled on native's `build_fun` (`lib/image_pipe/dialect/native.ex:376-409`). Preserve the runner's outward behavior: same `{:ok, {:prepared_stream, …}}` shapes to `Sender`, **same error taxonomy**.

  **Two corrections to this step (2026-07-15), from Task 3b's NEEDS_CONTEXT escalation:**

  1. **Task 3a changed the contract** — param 3 is now `Cache.Key.t() | nil` (pass `representation.cache_key`; `nil` is supported and stages nothing, which is what handles the framework's caching-off case — do **not** fabricate an `etag`/`vary`). `pump` is now **4-arity**: `pump.(stream, content_type, resolved_output, debug)`. Trace context and `cost_us` are **self-captured by the primitive** — do not thread them from `runner.ex`.
  2. **"Same error taxonomy" is binding, and it requires a `Delivery.Producer` change.** `source_session/producer.ex:435-441` translates a mid-stream `Source.StreamError` into `{:error, {:source, reason}}` on both the prepare and chunk paths; `delivery/producer.ex:162-168` hard-codes `{:encode, …}`, which `response/error_status.ex` classifies `:server_error` (500) instead of the source-domain class (422). Because `pump` owns the chunk loop, **the framework cannot restore this from its side** — a `Stream`'s `next_fun` cannot surface a tagged error. So teach `Delivery.Producer` about `StreamError` and add `ImagePipe.Source` to `Delivery`'s Boundary `deps:` (directionally clean — `Source` never names `Delivery`). Do **not** delete the pinning test (`source_session_test.exs:822`); port it. Reachability is not the criterion: the step says *same taxonomy*, and a 422→500 regression on a real error path is not a no-op even if only a test seam reaches it today.

  **Also note (found in the same escalation):** "exactly as native's `build_fun`" is not literally achievable — native passes constant debug, but the framework's `timings.encode` needs the first-chunk pull that `pump` owns, and handing `pump` the raw stream would collapse the `[:encode]` span, contradicting `docs/telemetry.md:248-254`. Resolve in-framework with a resumed `Stream.resource` (halt-at-position-0 still finalizes the encoder — verified). **The OTel parentage baseline does not catch this**, so it needs its own assertion.
...
> **Three additions from R4's review (2026-07-15), before the commits below.** R4's `operations/2` shipped as a partial port documented as a faithful one, producing four reachable divergences from the frozen framework arm (`enlargement`'s `:fill_down → :deny`, the dropped `zoom_*`/`min_*`/`down`/`guide` opts the carry math reads, the `{:auto,:auto,false}` emission guard, and `canvas_op`'s missing `extend_aspect_ratio_operation`). All four are fixed (`8349ec4e`) and both reviewers approved. **You inherit three obligations from that review:**
>
> 1. **Commit 9-0 FIRST — extract `ImagePipe.Dialect.Imgproxy.Assembly` before adding any stage.** This is the quality reviewer's concrete verdict, not a preference. `pipeline.ex` is now 730 lines holding three responsibilities the framework splits across `PlanBuilder`/`Executor`/`Resolver`: semantic op assembly (~330 lines, ~28 private fns — *where the fix pass just proved the bugs live*), the resolve-loop driver, and the carry math. Your stages (`missing_dimensions/1` guards, `orientation_operations/1`, the crop closure, nine more effects, `background_operations/1`) plausibly add 300–400 lines, putting it past 1000. **The seam is clean and proven now:** `operations/2` is a single entry point with a `PipelineRequest.t() -> [Plan.Operation.t()]` interface, so the extraction is mechanical today and progressively less so later. Move the assembly half only — the resolve-loop driver and the carry math are verbatim ports two reviewers verified; do not disturb them.
> 2. **Build the cross-arm comparison as a COMMITTED standing guard.** The fix pass verified itself with an 11-row dialect-vs-`PlanBuilder`+`Executor` comparison yielding byte-identical executables — but **threw it away**; `8349ec4e`'s message claims it as verification and it is not in the tree, so the claim is unfalsifiable and guards nothing. Both reviewers flagged this. Build it properly here, alongside the full assembly: it is the natural home, and it is the highest-value artifact this work can leave — a direct dual-arm gate over exactly the rows the differential suite compares. It must **discriminate** (fail if a stage diverges), not pass vacuously. Precedent for a cross-arm test that self-retires in phase 2: `leaf_structs_test.exs`.
> 3. **Close the two gaps R4's enumeration names, and keep the enumeration honest.** `pipeline.ex`'s moduledoc now enumerates what is *not* yet ported — that comment is the only thing that stopped this divergence shipping, so **as you land each stage, strike it from the list; never leave it claiming more than is done.** Two items need explicit attention: **(a) `missing_dimensions/1`'s guards** — the framework *rejects* those requests and the dialect currently **accepts them silently**, a real divergence; **(b) error propagation** — `plan_geometry/1` threads `{:ok,_} | {:error,_}` through every stage, while `operations/2` returns a bare list and uses `{:ok, op} = …`, so a constructor error **raises instead of becoming a 4xx**. Likely unreachable today (inputs are parser-validated), but your stages widen the input space; decide deliberately and say which.
