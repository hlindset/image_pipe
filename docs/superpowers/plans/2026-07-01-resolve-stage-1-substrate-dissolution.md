...
- **Stage 2 (spec §9):** boundary move of the imgproxy column, strategy behavioral version tags in `plan_material`, the `Lowering`/`ResizePlanning` re-signature to shape-based inputs (retires the `env.state` channel), focus → TwicPics strategy carry, `SourceShape`'s final boundary home, support-matrix update, closing #434. **Stage 2b (spec §4.4/§9):** de-dialect the Plan surface — `Plan.Operation.SetFocus` → generic strategy `%Directive{name, payload}`; `:carried` redefined as neutral strategy-supplied gravity.
...
- **R1 — lowering frame.** The resolver lowers via `Lowering.executable_operations(op, lowering_state, ctx)` where `lowering_state` starts from `env.state` (already overlaid from the shape by the driver, Task 5). For **flush-before** rows, rebuild it from the **post-Flush shape** (`pending_orientation: nil`, per-row shrink; its dims follow `effective_source_dims` semantics — the **unswapped** stored-original extent while shrink-on-load is still live, the quarter-turn-swapped display extent once shrink is consumed) so lowering — including `reject_region_out_of_bounds?` (lowering.ex:223-229, which reads `effective_source_dims` → prefers `source_dimensions`) — sees the frame exactly as today's post-flush lowering does. A blanket "swap the dims on a quarter turn" is **wrong** for the shrink-live case: today's flush never touches `source_dimensions`, so a post-flush region-bounds check under shrink-on-load reads the unswapped original. Declaring the lowering frame is the resolver's job; getting it wrong is the #182 class.
