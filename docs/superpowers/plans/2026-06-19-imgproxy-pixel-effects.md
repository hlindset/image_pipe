...
- [ ] **Step 2: Update `docs/imgproxy_support_matrix.md` "Background, effects, and overlays" table** — add `adjust`/`a` row (✅ Supported, "Meta-option → brightness/contrast/saturation; 1:1 imgproxy args"); flip `colorize`/`col` and `gradient`/`gr` to ✅ Supported with the compositing-contract + "Pro, pixels not bake-verified" note. For colorize/gradient, note explicitly that **opacity is clamped to `[0,1]` as an ImagePipe convention — the imgproxy docs state no upper bound** for these two (unlike monochrome/duotone), so this is inference by analogy + the blend model, not a doc-stated range. **Note:** the `brightness`/`contrast`/`saturation` rows are updated **in their own realignment tasks (Tasks 1–3)** per AGENTS.md's same-change conformance-doc rule — do NOT re-edit them here; just confirm they already read the realigned contract.
