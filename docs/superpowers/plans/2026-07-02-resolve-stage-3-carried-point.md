...
4. **A nil carried point at a `:carried` consumer falls back to the centred crop** — byte-identical to a plain centred crop, *including* the `center_bias` that `compensate_crop`'s `:carried` clause sets for the deferred-orientation odd-pixel discard (#146 Bug 2). This path is **hot**: the TwicPics parser's initial guide is `:carried` (`plan_builder.ex` `@initial`), so every plain `cover=`/`crop=` without a focus takes it. After Stage 3 the fallback is realized in **two** places, both preserved: `PointFlow`'s centre-anchor substitution (TwicPics plans) and a slimmed, point-free `:carried` → centre clause kept on `Crop.execute` (Task 5) — because the **native Plan constructors also accept `:carried`** (`Operation.resize_guide`/`tagged_crop_guide`) and a native, nil-strategy plan resolves through the `NeutralResolver` with no substitution, a supported public surface pinned by `focus_test.exs`'s #146 orientation matrix. `:carried` without a point-carrying strategy *means* the centred crop.
...
| `lib/image_pipe/transform/operation/crop.ex` | **New pure `resolved_rect/3`** shared with `execute` (Task 2); `:carried` execute clause slimmed to the point-free centre fallback (Task 5) |
...
Everything the Task-4 flow replaced is now dead **for point-carrying plans**: `State.carried_point` is never written (the directive emits zero ops), so every executable `Focus.*` call is a nil no-op, and no TwicPics crop reaching the chain carries `:carried` gravity. Delete the point channel — cleanly, per the AGENTS removal rule (no narration, no "no longer" notes) — with **one deliberate survivor**: the native Plan constructors (`Operation.resize_guide`/`tagged_crop_guide` in `lib/image_pipe/plan/operation.ex`) accept `:carried`, and a native nil-strategy plan resolves through the `NeutralResolver` with no substitution, so `Crop.execute` keeps a slimmed, **point-free** `:carried` clause meaning "the centred crop" (Pinned behavior 4). `Plan.KeyData`'s `guide_data(:carried)` also **stays** — TwicPics plans still carry `:carried` guides into cache-key material; only execution resolves them.
...
```
  ...
  - `crop.ex`: **slim** the `gravity: :carried` `execute/2` clause to the point-free centre fallback (no `Focus`/`State` point read — no point channel exists at execute time):

```elixir
  # `:carried` without a point-carrying strategy is the centred crop: native
  # plans resolve through the neutral resolver, which emits the gravity
  # unchanged. A point-carrying strategy (TwicPics) substitutes a concrete
  # gravity before emission, so a live point never reaches here.
  def execute(%__MODULE__{gravity: :carried} = params, %State{} = state) do
    execute(%__MODULE__{params | gravity: {:anchor, :center, :center}}, state)
  end
```

  Delete the `requires_materialization?(%__MODULE__{gravity: :carried})` clause (the catch-all `false` covers it); delete `carry_focus_through_crop/4` and inline its call away:
...
```
  ...
  Drop the `Focus` alias. `:carried` stays in the `gravity` typespec union (genuinely reachable for neutral plans).
  ...
  - `neutral_resolver.ex`: the `compensate_crop` `:carried` clause **stays** (it is live for both the TwicPics pre-substitution ops and the neutral native path); update only its comment's second paragraph to describe the nil-fallback in terms of the two consumers (the TwicPics strategy substitutes a nil point to the centred anchor; the executable's neutral `:carried` clause centres directly — both consume the `center_bias` set here; a set point becomes `:fp`, which ignores it).
...
- [ ] **Step 3: `focus_test.exs` disposition inventory.** `test/image_pipe/transform/focus_test.exs` exercises the deleted mechanics; apply these dispositions (do not over-delete — the `plan_builder`-based cases go through the real producer and stay):
  ...
  - `"nil-focus carried crop equals a centred crop under pending orientation"` (the `guided_crop_image` native-plan #146 Bug-2 orientation matrix) — **keep unchanged**: it pins the surviving neutral `:carried` centre fallback (Step 2's slimmed clause) across axis-reversing orientations and odd extents. It must stay green through this task; if it goes red, the slimmed clause is wrong — stop.
...
- **Plan-review cycle applied (2026-07-02, three parallel reviewers: TwicPics compatibility REQUEST-CHANGES; orientation/staging REQUEST-CHANGES; process/quality REQUEST-CHANGES).** All blockers and should-fixes incorporated:
  ...
  - *Native `:carried` producer* (staging blocker): the native Plan constructors accept `:carried` and resolve through the NeutralResolver with no substitution, pinned by `focus_test.exs`'s #146 orientation matrix — `Crop.execute` keeps a slimmed point-free `:carried` → centre clause (Task 5), preserving byte-identity for native plans, and the grep inventory/`focus_test.exs` disposition table was added (Task 5 Steps 1/3).
