...
- Modify: `lib/image_pipe/output/policy.ex` (public `identity_selection/1`), `lib/image_pipe/telemetry/trace/capture.ex` (`@safe_keys` + `:sig_key_index`), `docs/telemetry.md`
- Test: `test/image_pipe/dialect/native_wire_test.exs`, `test/image_pipe/output/policy_identity_selection_test.exs`, a Capture test for the new metadata key
