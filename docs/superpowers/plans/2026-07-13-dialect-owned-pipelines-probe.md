...
def call(conn, config) do
  with :ok <- Signature.verify(conn, config),                          # 403 before parse
       {:ok, request} <- Parser.parse(conn, config),                   # canonical %Native.Request{} | {:invalid, diagnostics}
       ...
       {:ok, selection} <- negotiate(conn, request, config),           # selected format + vary?
...
lib/image_pipe/dialect/native/identity.ex             identity_material/4 → Representation.IdentityMaterial
...
Canonicalization rules the parser must apply (property-tested in Task 5): within a group option order is irrelevant (struct fields make this structural); Tier-1 identity values (`blur=0`) normalize to the absent field; equivalent spellings normalize (`fff` ≡ `ffffff` ≡ `white` → the same `{255,255,255}` tuple; ratio never applies in the subset); **semantic defaults normalize to the absent field** — explicit `fit=contain` (with a resize dimension present) ≡ absent `fit`, explicit `anchor=center` (with a consumer present) ≡ absent guide, so `/w=800` and `/fit=contain/w=800` yield equal `%Native.Request{}` (and later, identical identity) [native §Geometry semantics defaults]. The canonical value is pure data — no PIDs/refs/conn state [pipelines §Design principles 2].
...
defmodule ImagePipe.Representation.IdentityMaterial do
  ...
  #                                             normalized negotiation outcome (:selected_format),
...
Both digests go through `ImagePipe.MaterialDigest`. Dialects never concatenate key material. Negotiation enters as the selected variant (`selected_format: :avif`), never the raw `Accept` value [pipelines §Enforcement model 2].

### Native identity table (drives Task 10; verbatim from [native §Canonical form and identity])

| Input | Category |
| --- | --- |
| transform groups, terminal (`output`), `q` | `representation` |
| negotiated selected format (or the explicit `format`) | `representation` |
...
3. **Presets minimal:** named single-group presets + `default` preset, strict precedence chain, unknown-preset 400, cache-key transparency. Override families, disable pruning, and multi-group presets land post-probe.
...
Rules (each a test): non-empty query string → error; `sig=` only valid first; `src`/`src64` terminal (missing → error, never a source guess); `src` tail percent-decoded exactly once, malformed escapes → error (post-verify, but lexer reports); `src64` exactly one unpadded url-b64 segment (embedded `/` or `=` → error); percent escapes in option segments → error; empty segments (duplicate slashes) and dot segments → error.
...
`%OptionSpec{key, scope (:group | :request), value (parser fun or :flag), stage, prerequisites, conflicts, identity (:representation | :gate), terminal_applicability, summary, examples}` — one struct per probe-subset option (`w h fit enlarge crop region anchor focus blur trim pad bg output format q expires preset`), driving key lookup, scope/duplicate validation, value dispatch, and the completeness test ("every option declares all fields plus ≥ 1 example") [native §Architecture, option schema].
...
4. cross-option over successfully parsed values only (derivative suppression [native §Error diagnostics]): Tier-3 exclusives `focus`+`anchor`, `crop`+`region`; Tier-2 inertness (subset rows): `anchor`/`focus` need a consumer (`crop`, or `fit ∈ {cover, cover-down, auto}`), `enlarge` needs a resize intent (`w`/`h`).
...
Rules, each a test [native §Signing, §Byte-level contract]: MAC = HMAC-SHA256 over the raw bytes after the sig segment (from the following `/` to end of path, query excluded, mount-relative — Task 4's `signed_path`); unpadded url-b64, exactly 43 chars, non-canonical encodings rejected; ordered key list, first signs, each tried with `Plug.Crypto.secure_compare/2`; keys configured + missing/invalid → `:missing_signature`/`:invalid_signature` (→ 403, terse body, no spans); no keys + `sig=` present → `:signature_without_keys` (→ 400); no keys + no sig → `:ok`. Matched key index lands in `[:parse]` stop metadata as `:sig_key_index` (nil when unsigned) — update `Trace.Capture` `@safe_keys`, `docs/telemetry.md`, and a capture test in the same task (Logger needs no change: metadata-only). Property: sign → verify round-trip over arbitrary option paths. Ordering test: a signed path containing a duplicate slash **verifies** (MAC over the raw bytes as sent) and only then 400s at parse (empty segment) — 400, never 403 [native §Byte-level contract]. `expires` (request-scoped option from Task 5): past unix timestamp → 404 gate in the chain; not identity material.
...
**Interfaces:**
- Consumes: `%Native.Request{}`, the negotiation selection — **this task owns the shape**: `selection :: %{selected_format: atom(), vary?: boolean()}` (`selected_format` may be the `:source_negotiated` sentinel; Task 15's `negotiate/3` later produces exactly this map), `Representation.storage_inputs/2`.
- Produces: `Native.Identity.material(request, selection, conn, config) :: Representation.IdentityMaterial.t()` and `@dialect_epoch 1` exposed as `{ImagePipe.Dialect.Native, 1}`.

Composition per the identity table in the Design Reference. The `representation` keyword must be built from the canonical request only (groups + output + q + selected_format) — a test asserts `expires`/signature never appear in it; `conn` contributes only via `storage_inputs`.
...
- [ ] **Step 1: Failing tests** —
  ...
  - explicit `format` → `selected_format` is that format regardless of `Accept`
  - blurhash terminal: `format`-family keys are absent from the subset's inert rules only when applicable — assert terminal enters `representation` and that an `:image`-vs-`:blurhash` request differs
...
**Files:**
- Create: `lib/image_pipe/decode.ex`, `lib/image_pipe/decode/decoded.ex`, `lib/image_pipe/transform/source_geometry.ex`
...
**Interfaces:**
- `negotiate(conn, request, config)`: explicit `format` → `%{selected_format: fmt, vary?: false}`; absent → build `%Plan.Output{mode: :automatic, quality: …}` → `Output.Policy.from_output_plan/3` + `ensure_capable/2` (encode-time resolution stays `Policy.resolve/2` post-decode). **Pre-fetch identity normalization rule** (identity needs the selection before any fetch; `Policy.resolve_before_source_fetch/1` is private — do NOT call it; the `%Output.Policy{}` struct is exported, so `Native.negotiate` reads `policy.mode` / `policy.modern_candidates` directly and makes the three-line decision itself, citing `lib/image_pipe/output/policy.ex:106-116` as reference):
  - explicit `format` → that format;
  - non-empty `modern_candidates` → the list **head** (the format that will actually be encoded);
  - empty `modern_candidates` → the fixed sentinel `selected_format: :source_negotiated` — **never** a concrete source-derived format, which is unknowable pre-fetch. Sound because the eventual jpeg-vs-png divergence is a deterministic function of `source_identity` + groups + core epoch, all already in the identity.
  Normalize to the **single selected format only** — do NOT carry the candidate list into `representation`. The framework's `HTTPCache.accept_material/3` keys the whole candidate list; that is the old behavior this design deliberately replaces ("two headers that both negotiate AVIF share a cache entry and an ETag" [pipelines §Enforcement 2]) — do not mirror it. Acceptance: the contract-kit `same_selection` cases, including the no-modern bucket (Task 18).
...
- [ ] **Step 1: Failing wire tests** (each a named test; rig per Design Reference):
  ...
  - `/q=42/...` differs in bytes from `/q=90/...` (representation identity carries q)
  - cache reuse: two requests, `CacheProbe` sees one `:cache_put` then one `:cache_lookup` hit; permuted-option URL hits the same key
...
**Files:** Modify `lib/image_pipe/dialect/native/pipeline.ex` (terminal computation), `lib/image_pipe/dialect/native.ex` (complete-body generate branch), `lib/image_pipe/cache.ex` + `lib/image_pipe/cache/sink.ex` + `lib/image_pipe/cache/entry.ex` (complete-body widening below); Test `test/image_pipe/dialect/native/blurhash_test.exs` (unit), `test/image_pipe/cache/complete_body_sink_test.exs`, extend `native_wire_test.exs`.

Semantics [native §Terminal contracts]: after all groups + flush, reduce internally (working frame ≈32px: emit a final contain-resize to fit 32×32 via the same lowering path), then normalize to the **fixed terminal pixel space** — sRGB, tone-mapped, independent of the decoded image's color profile (this is a fixed conversion, not an option; it is byte-affecting for the hash and pinned by the terminal's behavior version — record in Task 21.6), then `Image.Blurhash.encode(image, x_components: 4, y_components: 3)` (exact arity/signature recorded in Task 1) → body = the BlurHash string; `content-type: text/plain; charset=utf-8`; **no** `Vary`; `format`/`q` inert-with-terminal rules are out of subset scope (only `q` exists; per [native §Applicability] `q` is Tier-2 inert with a non-image output — reject `q`+`output=blurhash` as inert, add the OptionSpec rule + parser test back in Task 5's file). Identity: terminal + groups in `representation`; `selected_format` absent (test in `identity_test.exs`). Delivery: complete body — no producer; respond via `Plug.Conn.send_resp/3` with ETag/CL headers (`Sender`'s `{:rendered, …}` shape does JSON negotiation — not a fit; keep dialect-owned send here and note it in the exports report).

**Complete-body cache write requires a scoped core widening** — the sink and entry are image-format-coupled at three verified points: `Sink.open` derives content type via `Format.mime_type!(resolved_output.format)` (raises for non-image), `Entry.Metadata` stores an image `output_format`, and `Entry.validate_content_type` rejects anything that doesn't map back to an image format — so a text body cannot pass through `Cache.open_sink(%Key{}, %Output.Resolved{}, opts)` today. Widen (files: `lib/image_pipe/cache.ex`, `lib/image_pipe/cache/sink.ex`, `lib/image_pipe/cache/entry.ex`): a `Cache.open_sink/3` clause accepting `{:complete_body, content_type :: String.t()}` in place of the `%Output.Resolved{}`, an `Entry.Metadata` variant carrying the literal content type (`output_format: :complete_body`), and an `Entry` validation branch admitting that variant. Tests: entry round-trip with `text/plain; charset=utf-8`; fail-open preserved on sink errors; existing image-entry validation unchanged. Record the widening in Task 21.6.
...
A synthetic TwicPics-like ordered interpreter [pipelines §The probe]: a command list like `[{:resize_w, 500}, {:crop_rel, 0.5, 0.5}, {:resize_w, 200}]` evaluated left-to-right with carried local state `%{dims: {w, h}}` — per command: compute executable op against *current measured* dims → `Chain.execute` → measure → update state. Plus `preflight/2`: walk the same command list purely over header dims, propagating a **conservative lower bound** on required decode dims (relative ops propagate; absolute ops reset; unknown-until-measured ops (a trim analog) collapse the bound to "no shrink") → `DecodePlanner.Request`.

- [ ] **Step 1: Failing tests** — interpreter correctness (dims after each command over a fixture, both orientations); preflight soundness property (StreamData over random command lists: decode with preflight shrink → final pixel dims equal the no-shrink run's); a documented case where no useful bound exists (leading trim) → preflight yields no shrink, and the test records the measured cost delta (decode dims full vs potential) via an assertion on the plan, not wall time.
...
1. **Hop-count/concept-count comparison**: the same request (`/w=800/src/images/cat.jpg` and its imgproxy equivalent) traced through both stacks — enumerate hops (module→module transitions) and concept families; baseline from `docs/execution_flow.md` (~14 hops / ~8 families) vs the native chain, counted from the code as built.
...
6. **Core-exports list**: every core module/function the probe consumed (grep the dialect + kits for `ImagePipe.` references), grouped: pre-existing exports, exports widened (`SourceGeometry`, `DecodePlanner.Request`/`open_options_for`, `Conditional`, `lookup_entry`, `with_fetched`), new boundaries (`Representation`, `Decode`), and duplicated-not-extracted (delivery session/producer, Processor decode internals, conditional-match logic) — the draft of the real toolkit surface, with a recommendation per duplicate (extract / keep duplicated / N/A).
...
- **Resolved during plan review** (do not reopen): pre-fetch selected-format identity = explicit format / candidate head / `:source_negotiated` sentinel, single format only, never the candidate list (T15); cover staged tails execute unchanged, measured dims only advance the shape, and the dialect drives `NeutralResolver.resolve/3` + `continue/4` directly as stateless toolkit calls (T14); the complete-body cache write requires the T17 sink/entry widening; `If-None-Match: *` honored on the cache-hit path only (T16).
