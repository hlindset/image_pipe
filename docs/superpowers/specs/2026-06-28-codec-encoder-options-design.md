...
- `Cache.Key` — the three `jxl_effort` literals (`output_plan_data` ×2 clauses + `output_data`) are replaced by `encoder_options` (structs flattened to plain maps via `encoder_options_key/1` so `MaterialDigest.canonicalize/1` can enumerate them — structs aren't `Enumerable`). **Per-mode shape (refined post-review, CodeRabbit):** the **automatic** clauses carry the **whole** map (the output format is negotiated post-fetch, so any format's options may shape the bytes); the **explicit** clause narrows to `Map.take(encoder_options, [format])`, because `Policy.resolved/2` forwards only `Map.get(encoder_options, format)` to the encoder — so two explicit-`:png` requests differing only in unrelated `:webp_options` are byte-identical and must share a cache entry/ETag. (This is a deliberate asymmetry with `format_qualities`, which still carries its whole map in the explicit clause — a pre-existing benign over-keying left unchanged to avoid touching that field's established cache behavior in this PR.)
