...
**maxArea must never be exceeded (it's a MUST, not a SHOULD).** Per-axis `positive_round` of a derived scale can push `w·h` marginally *above* `max_area` — and not only when the area term binds: a binding **axis** term rounded up can also overshoot when an axis and the area co-bind (verified counterexample: source 7622×1774, `max_width=max_height=3874`, `max_area=3_494_075` → 3874×902 = 3_494_348 > max_area). So **floor both axes whenever any `max_area` is configured and a real scale applies**, regardless of which term binds (`area_bounded?/1` keys on `max_area != nil`). Flooring keeps `trunc(w·s)·trunc(h·s) ≤ (w·s)(h·s) ≤ max_area`. A combined-bounds property test (all three bounds set at once, the IIIF path) asserts `w·h ≤ max_area` and the axis caps across shapes.
...
- `lib/image_pipe/transform/operation/resize.ex` — `defstruct` + `@type` add the fields; `resolve_dimensions/2` applies the **per-candidate** grow/clamp bounding step to `intermediate` and `target` (new private helpers `apply_bounds/3` + `bound_scales/2` + `area_bounded?/1` + the grow-vs-clamp predicate), with `:auto`-axis pass-through, an all-nil fast path, and **floor both axes whenever `max_area` is configured** (not only when the area term binds) so `w·h ≤ max_area` holds exactly.
