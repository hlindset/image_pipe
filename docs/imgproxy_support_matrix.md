...
- ⚠️ `IMGPROXY_AUTOQUALITY_TARGET` — `autoquality_target` (SSIMULACRA2 score for `:ssimulacra2`, butteraugli distance for `:butteraugli`, byte count for `:size`; **not** a DSSIM value). **Metric-aware default:** `:ssimulacra2` defaults to `78` (the SSIMULACRA2 "very high quality" tier, ≈ imgproxy's DSSIM-`0.02` *intent* re-expressed on this scale — imgproxy's literal `0.02` is meaningless here); `:butteraugli` defaults to `@default_butteraugli_target 1.0` (a butteraugli distance ≈ visually lossless; **lower-is-better**, range `0.0..25.0`); `:size` has no sane byte default, so it stays required (a request with `:size` active and no URL/config target fails with an invalid-option error). The `78`/`1.0` defaults are **global** — there is deliberately no per-format target (see the note below).
- ✅ `IMGPROXY_AUTOQUALITY_MIN` — `autoquality_min_quality` (default `70` for `:size`/`:ssimulacra2`; **`:butteraugli` defaults to the full-range bracket `1`** so the quality clamp only bites when the user sets it explicitly).
- ✅ `IMGPROXY_AUTOQUALITY_MAX` — `autoquality_max_quality` (default `80` for `:size`/`:ssimulacra2`; **`:butteraugli` defaults to the full-range bracket `100`**).
- ⚠️ `IMGPROXY_AUTOQUALITY_ALLOWED_ERROR` — `autoquality_allowed_error` (default `1.0`; a **symmetric** tolerance band **on the SSIMULACRA2 scale** — the `:ssim2` objective walks toward `target` and accepts any quality scoring within `[target − allowed_error, target + allowed_error]`, stopping at the first in-band quality rather than minimizing quality below the band). **Scale collision, documented:** this is **not** imgproxy's DSSIM `allowed_error` (0–1, where `1.0` means "accept anything"); on the SSIMULACRA2 scale the same `1.0` is a strict ±1-point band around the target, and imgproxy's own default `0.002` would be a near-exact-match band. Applies to `:ssimulacra2` and `:butteraugli` (`:size` is a bare `bytes ≤ target` predicate with no band). **`:butteraugli` band:** the tolerance is on the **distance** scale and **lower-is-better**, so the search walks toward `target` from the opposite direction; its default `allowed_error` is `0.1` (a distance band), not `1.0` — `1.0` would be enormous on the distance scale.
...
**Single global target, no per-format target (matches imgproxy's structure).** Like imgproxy, ImagePipe carries one global `autoquality_target` and lets the per-format quality **caps** absorb each codec's quality-number differences (`avif=60/65` already overrides the `70/80` base) — there is no `autoquality_format_target`. The `:ssim2` default (`78`) was set by measuring what each default cap actually delivers on representative content (a photographic + screenshot/synthetic corpus) and placing the target below the **tightest** ceiling so the search lands in-bracket rather than pinning best-effort. **`:butteraugli` (ImagePipe extension)** shares this single-global-target structure but is **lower-is-better** (distance, default `1.0`) and **full-frame only** this cycle — there is no crop-scored fast path above the ~6 MP crossover (that approximation is `:ssimulacra2`-only), and its default bracket is full-range `1..100` so the per-format caps and the quality clamp only constrain it when set explicitly. Findings that shaped the `:ssimulacra2` default:
