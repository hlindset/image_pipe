...
suite both assert every case against imgproxy-baked fixtures. The "Realized
in" column below names where each stage is realized.

A handful of ImagePipe-side implementation facts — cache keys, ETags,
telemetry — are not imgproxy surfaces at all. They're enumerated in
...
imgproxy-conformance gap — imgproxy itself has no server-side cache and no
telemetry surface to diverge from — but a host that mounts the dialect should
know them:

### Behavioral

- **`/info` is cached.** A dialect-owned complete-body cache entry, honoring
...
`signature: [keys: [...], salts: [...], signature_size: n, trusted_signatures: [...]]`.
...
Custom `source_schemes: ...` translators can map more schemes to
...
`autoquality_method: ..., ...` (URL args override per request). The two
...
ImagePipe supports URL `auto_rotate` and the matching dialect config default:
`auto_rotate: true`, which is also the default. URL `strip_metadata`,
`keep_copyright`, and `strip_color_profile` are supported with dialect-owned
...
- ✅ `IMGPROXY_STRIP_METADATA` — Dialect config default: `strip_metadata: true`. URL override: `sm:0` disables. Strips EXIF, XMP, and IPTC at encode time via `ImagePipe.Plan.Output` metadata policy.
- ✅ `IMGPROXY_KEEP_COPYRIGHT` — Dialect config default: `keep_copyright: true`. URL override: `kcr:0` disables. **Diverges from imgproxy**: preserves EXIF copyright/artist fields only; imgproxy retains full XMP/IPTC blobs. ImagePipe strips XMP/IPTC even when `kcr` is on (privacy-conservative).
- ⭕ `IMGPROXY_STRIP_METADATA_DPI`
- ✅ `IMGPROXY_STRIP_COLOR_PROFILE` — Dialect config default: `strip_color_profile: true`. URL override: `scp:0` disables. Sets `Plan.Output.color_profile`: `scp:1` → `:strip` (convert to standard space, drop the profile at encode); `scp:0` → `:preserve_source` (re-embed the source profile on output). Input working-space conditioning is unconditional and not controlled by this option — every profiled/wide-gamut/CMYK source is imported to a working space before processing via the `InputColorManagement` preamble, matching imgproxy's `colorspaceToProcessing`. Encoder finalize handles re-embed or drop per the policy (stage 16). Dropping the profile (`scp:1`) also strips the three EXIF color-characterization tags that imgproxy's `vips_icc_remove` removes (`exif-ifd0-WhitePoint`, `exif-ifd0-PrimaryChromaticities`, `exif-ifd2-ColorSpace`), independent of `sm` — see stage 16.
...
`storage_inputs: [{:header, name}, {:cookie, name}]`, validated by
`ImagePipe.Dialect.Imgproxy.validate_config!/1` and folded into the cache key by
`ImagePipe.Dialect.Imgproxy.Identity`; configured header names also enter
`Vary`. ImagePipe has no built-in cloud cache adapters.

- ✅ `IMGPROXY_CACHE_USE`
- ✅ `IMGPROXY_CACHE_FS_ROOT`
- ✅ `IMGPROXY_CACHE_PATH_PREFIX`
- ⭕ `IMGPROXY_CACHE_BUCKET`
- ✅ `IMGPROXY_CACHE_KEY_HEADERS` — mount option `storage_inputs: [{:header, name}]`
- ✅ `IMGPROXY_CACHE_KEY_COOKIES` — mount option `storage_inputs: [{:cookie, name}]`
...
| AES-CBC source URL encryption helpers | Supported | `ImagePipe.Dialect.Imgproxy.encrypt_source_url/3` returns the segment used after `/enc/`. The helper doesn't build full paths, output suffixes, or signatures. |
...
| `auto_rotate` | `ar` | Supported | Omitted argument enables auto-orient; boolean form supported. URL `ar` overrides the `auto_rotate: ...` dialect config request-wide, with last value in path order winning. |
...
| Imgproxy option | Aliases | Status | Notes |
| --- | --- | --- | --- |
| `strip_metadata` | `sm` | Supported | Default on. Dialect config: `strip_metadata: true`. URL override `sm:0` disables. Strips EXIF, XMP, and IPTC at encode time via `ImagePipe.Plan.Output` metadata policy. |
| `keep_copyright` | `kcr` | Supported | Default on. Dialect config: `keep_copyright: true`. URL override `kcr:0` disables. **Diverges from imgproxy**: preserves EXIF copyright/artist fields only; imgproxy retains full XMP/IPTC blobs. ImagePipe strips XMP/IPTC even when `kcr` is on (privacy-conservative). |
| `dpi` | | Missing | Pro metadata rewrite. |
| `strip_color_profile` | `scp` | Supported | Default on. Dialect config: `strip_color_profile: true`. URL override `scp:0` disables. Sets `Plan.Output.color_profile`: `scp:1` → `:strip`; `scp:0` → `:preserve_source`. Input color conditioning runs unconditionally (not controlled by this option): the `InputColorManagement` preamble imports every profiled source to a working space before processing, matching imgproxy. The encoder finalize re-embeds the source profile (`scp:0`, format supports profiles) or drops it (`scp:1`). Dropping also strips the three EXIF color-characterization tags imgproxy's `vips_icc_remove` removes (`WhitePoint`, `PrimaryChromaticities`, `ColorSpace`), independent of `sm` (stage 16). |
...
| Imgproxy feature | Status | Notes |
| --- | --- | --- |
| Named presets | Supported | Configured through `presets: %{name => options}`. Expanded while parsing normal processing URLs. |
