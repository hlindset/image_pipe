...
- ⚠️ `IMGPROXY_AUTOQUALITY_ALLOWED_ERROR` — `autoquality_allowed_error` (default `1.0`; a tolerance band **on the SSIMULACRA2 scale** — the `:ssim2` objective accepts the lowest quality scoring `≥ target − allowed_error`). **Scale collision, documented:** this is **not** imgproxy's DSSIM `allowed_error` (0–1, where `1.0` means "accept anything"); on the SSIMULACRA2 scale the same `1.0` is a strict one-point band, and imgproxy's own default `0.002` would be a near-exact-match band. Applies only to `:ssim2` (`:size` is a bare `bytes ≤ target` predicate with no band).
