...
- ⚠️ `IMGPROXY_JXL_EFFORT` — `jxl_effort` (JPEG XL encode effort, `1..9`, **no URL form**).
  **Diverges (surface + behavioral):** ImagePipe's neutral default is **7** (libvips
  ...
  empty today, so the two ship different JXL `effort` and thus different JXL bytes; set
  the overlay's `jxl_effort: 4` for byte-parity. See the "Save / encode" row.

**Config ownership.** The product-neutral plan/output tunables above (`quality`,
`format_quality`, the `autoquality_*` family, `jxl_effort`, the metadata/color flags)
are owned by the neutral `ImagePipe.Config` boundary, not the imgproxy adapter; the
adapter validates only its dialect keys (`signature`, `source_url_encryption_key`,
`base64_url_includes_filename`, `source_schemes`, `presets`) and delegates the neutral
keys to `Config`, merging a sparse **parity overlay** on top of the neutral defaults.
**imgproxy parity == neutral defaults today** — the overlay is empty (the lone
value-level divergence, `jxl_effort` 7 vs 4, is documented above rather than overlaid).
It doesn't expose codec-specific knobs beyond quality.
...
- ⚠️ `IMGPROXY_AUTOQUALITY_TARGET` — `autoquality_target` (**per-metric map**; SSIMULACRA2 score for `:ssimulacra2`, butteraugli distance for `:butteraugli`, byte count for `:size`; **not** a DSSIM value). Resolution per metric: URL arg → config map entry → built-in default. **Built-in defaults:** `:ssimulacra2` → `78` (the SSIMULACRA2 "very high quality" tier, ≈ imgproxy's DSSIM-`0.02` *intent* re-expressed on this scale — imgproxy's literal `0.02` is meaningless here); `:butteraugli` → `@default_butteraugli_target 1.0` (a butteraugli distance ≈ visually lossless; **lower-is-better**, range `0.0..25.0`); `:size` has no sane byte default, so it stays required (a request with `:size` active and no URL/config target fails with an invalid-option error). Each map entry is range-checked against its own metric at the config boundary. There is deliberately no per-format target (see the note below).
