...
| `colorize` | `col` | ✅ Supported | `col:%opacity:%color:%keep_alpha` — opacity from `0` to `1` (`0` no-op), optional hex color (default `000`), optional `keep_alpha` boolean (default `false`). Composites a flat color over the source: `out = src·(1−o) + color·o`. With `keep_alpha`, the source alpha channel is preserved through the blend. **Opacity is clamped to `[0,1]` as an ImagePipe convention** (imgproxy docs state no upper bound for colorize/gradient opacity). **Pro-only (no OSS wire-diff):** imgproxy `colorize` is Pro; ImagePipe-implemented, not OSS parity — pixels not bake-verified. |
