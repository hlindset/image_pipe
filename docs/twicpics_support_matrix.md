...
| `focus=auto` | ✅ Supported | Content-aware subject detection → the `:smart` guide for the next `cover` / `crop`; emits no operation. **Diverges (behavioral):** TwicPics `auto` uses ML subject detection; ImagePipe approximates it with libvips attention smart crop (`VIPS_INTERESTING_ATTENTION`) — the same content-aware approximation used for imgproxy `g:sm`. |
