...
| `anchor` | eight documented literals (plus `center` for `focus`) | `top`, `bottom`, `left`, `right`, `top-left`, `top-right`, `bottom-left`, `bottom-right`. The documented `anchor` parameter list omits `center`, but **live `focus` accepts `center`** (verified: `focus=center` returns `200`; a bogus `focus=middle` returns `404`), resolving it to the centre point (= the default focus). The overlay path-config anchor list documents `center` explicitly. |
