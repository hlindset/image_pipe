...
- **Marker accretion:** a dialect marker may live on a shared `ImagePipe.Plan` struct (a field value only the emitting dialect's resolver understands, e.g. the `:deferred` guide TwicPics puts on `Resize`/`CropGuided`, resolved to a concrete point by `ImagePipe.Parser.TwicPics.Resolver`'s focus carry) only when the decision it defers is **all three of** (a) runtime-geometry-dependent (undecidable at parse time, before any source fetch), (b) per-operation/positional (its meaning depends on where it sits in the pipeline, not just on config), and (c) has **no product-neutral specification** — i.e. you cannot state its behavior without naming the dialect. If any one fails, don't carry a marker: promote the rule into the neutral column (`ImagePipe.Transform.NeutralResolver`) or the relevant `Plan` struct so every dialect gets it and no resolver is required. Resize `mode: :auto` was the worked example of (c) failing — its fill-vs-fit rule *is* statable product-neutrally (fill when source and target orientation match on the display axes, fit otherwise), so it was promoted to the neutral resolver (#448) rather than staying imgproxy-strategy vocabulary. Provenance (imgproxy, `color.White`) is not the same as dialect-specificity.
