...
- When presenting the inline-vs-subagent execution choice for a plan, lead with a calibrated recommendation rather than a neutral menu. Default to **inline execution + one final parallel review of the complete diff** when the plan's tasks form a sequential dependency chain in a few shared files, stay within one boundary, and are holdable in a single working context — there the subagent-per-task handoff adds overhead without parallelism, and incremental TDD catches integration bugs faster than per-task review round-trips. Reserve **subagent-per-task with two-stage review** for plans whose tasks are genuinely independent/parallelizable, span many files or boundaries, or are large enough that context isolation per task earns its keep. Flag any task that is environmental/interactive (network fetch, native build, a judgment call on measured numbers) as ill-suited to a subagent and better run inline regardless of the overall choice.
