...
- Keep `deps:` aligned with architecture direction. One line per namespace:
  - `parser` → `plan`, `renderer`, `transform` (a host `ImagePipe.Parser`
    produces only product-neutral declarative Plans; the neutral
    runtime-geometry lowering under `transform/*` owns every source-dependent
    geometry decision, so parsers never carry a geometry-resolution strategy.
    Ordered, product-specific runtime carry lives in a self-contained dialect
    Plug, not a parser)
