defmodule ImagePipe.API.Config do
  ...
  def validate!(opts) when is_list(opts) do
    {source_encryption_keys, opts} = Keyword.pop(opts, :source_encryption_keys, [])
    source_encryption = validate_source_encryption_keys!(source_encryption_keys)
    {signing_keys, opts} = Keyword.pop(opts, :keys, [])
  ...
  defp bool!(key, field, value) do
    raise ArgumentError,
          "invalid ImagePipe.API options: #{key} #{field} (#{inspect(value)}) must be a boolean"
  end
  ...
  defp int_range!(key, field, value, lo..hi//_) do
    unless is_integer(value) and value >= lo and value <= hi do
      raise ArgumentError,
            "invalid ImagePipe.API options: #{key} #{field} (#{inspect(value)}) must be in #{lo}..#{hi}"
    end
  end
  ...
  defp enum!(key, field, value, allowed) do
    unless value in allowed do
      raise ArgumentError,
            "invalid ImagePipe.API options: #{key} #{field} (#{inspect(value)}) must be one of #{inspect(allowed)}"
    end
  end
  ...
  defp validate_quality_value!(key, value) do
    unless value in 1..100 do
      raise ArgumentError,
            "invalid ImagePipe.API options: #{key} (#{value}) must be between 1 and 100"
    end
  end
  ...
  defp validate_quality_map!(key, map) do
    Enum.each(map, fn {format, quality} ->
      unless Format.output_format?(format) do
        raise ArgumentError,
              "invalid ImagePipe.API options: #{key} has unsupported format #{inspect(format)}"
      end

      unless quality in 1..100 do
        raise ArgumentError,
              "invalid ImagePipe.API options: #{key} #{inspect(format)} (#{quality}) must be between 1 and 100"
      end
    end)
  end
  ...
  defp validate_target_metric!(:size, value) do
    unless is_integer(value) and value > 0 do
      raise ArgumentError,
            "invalid ImagePipe.API options: autoquality_target :size (#{inspect(value)}) must be a positive integer"
    end
  end
  ...
  defp validate_target_metric!(metric, value) when metric in @perceptual_metrics do
    ...
    unless is_number(value) and value >= low and value <= high do
      raise ArgumentError,
            "invalid ImagePipe.API options: autoquality_target #{inspect(metric)} (#{inspect(value)}) is out of range #{inspect({low, high})}"
    end
  end

  defp validate_target_metric!(metric, _value) do
    raise ArgumentError,
          "invalid ImagePipe.API options: autoquality_target has unknown metric #{inspect(metric)}"
  end
  ...
  defp validate_allowed_error!(error_map) do
    Enum.each(error_map, fn {metric, value} ->
      unless metric in @perceptual_metrics do
        raise ArgumentError,
              "invalid ImagePipe.API options: autoquality_allowed_error has unsupported metric #{inspect(metric)}"
      end

      unless is_number(value) and value >= 0 do
        raise ArgumentError,
              "invalid ImagePipe.API options: autoquality_allowed_error #{inspect(metric)} (#{inspect(value)}) must be a non-negative number"
      end
    end)
  end
  ...
  defp validate_brackets!(resolved) do
    ...
    if base_min > base_max do
      raise ArgumentError,
            "invalid ImagePipe.API options: autoquality_min_quality (#{base_min}) exceeds autoquality_max_quality (#{base_max})"
    end
    ...
    |> Enum.each(fn format ->
      ...
      if effective_min > effective_max do
        raise ArgumentError,
              "invalid ImagePipe.API options: effective autoquality bracket for #{inspect(format)} is inverted " <>
  ...
  defp validate_source_encryption_keys!(keys) do
    case SourceEncryption.new(keys) do
      ...
      {:error, message} ->
        raise ArgumentError, "invalid ImagePipe.API source_encryption_keys: #{message}"
    end
  end
  ...
  defp validate_signing_keys!(keys) when is_list(keys) do
    if Enum.all?(keys, &match?({:ok, _key}, validate_hex_key(&1))) do
      ...
      raise ArgumentError, "invalid ImagePipe.API signing keys"
    end
  end
